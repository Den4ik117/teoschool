...
@section('header')
    <div class="flex items-center justify-between py-2">
        <div class="font-bold">
            <span>Создание задания для {{ $user->person->grade->who() }}</span>
            <a class="text-indigo-400 font-bold hover:underline" href="{{ route('class.persons.show', $user->slug) }}">[назад]</a>
        </div>
        ...
        <div class="md:grid md:grid-cols-3 md:gap-6">
            <div class="md:col-span-1">
                <div class="px-4 sm:px-0">
                    <h3 class="text-lg font-medium leading-6 text-gray-900">Создание задания</h3>
                    <p class="mt-1 text-sm text-gray-600">Здесь вы можете создать задание для {{ $user->person->grade->who() }}</p>
                </div>
            </div>
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form action="{{ route('class.persons.exercises.store', $user->slug) }}" method="POST">
                    @csrf
